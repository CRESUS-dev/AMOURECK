...
def package_dashboard_data(request):
    ...
    if date_from:
        d_from = parse_date(
            date_from
        )  # transforme "2025-10-01" → datetime.date(2025, 10, 1)
        dt_from = make_aware(
            datetime.combine(d_from, time.min)
        )  # crée un datetime à 00:00:00
        qs = qs.filter(
            updated_at__gte=dt_from
        )  # rend ce datetime “timezone-aware” (compatible avec les champs DateTimeField)
