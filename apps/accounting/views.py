...
def accounting_table(request):
    ...
    for ag in agencies:
        ...
        total_ch = ops_ag.filter(operation_type="CH").aggregate(total=Sum("amount"))["total"] or 0
        total_pd = ops_ag.filter(operation_type="PD").aggregate(total=Sum("amount"))["total"] or 0
