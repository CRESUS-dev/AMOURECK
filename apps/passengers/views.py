...
def ticket_render_pdf_view(request, *args, **kwargs):
    ...
    for field, value in ticket.__dict__.items():
        clean_ticket[field] = (
            "" if value is None or str(value).lower() == "none" else value
        )
    ...
    if ticket.agency.logo:
        ...
        if os.path.exists(logo_path):
            logo_url = request.build_absolute_uri(
                settings.MEDIA_URL + str(ticket.agency.logo)
            )

    context = {"ticket": ticket, "logo_url": logo_url, "clean_ticket": clean_ticket}
    ...
    if pisa_status.err:
        return HttpResponse(
            "Erreur lors de la génération du PDF.<pre>" + html + "</pre>"
        )
...
def ticket_dashboard_data(request):
    ...
    counts = (
        qs.values("agency__name").annotate(total=Count("id")).order_by("agency__name")
    )
